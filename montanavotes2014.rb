...
while true 
  ...
  while z <= 4
    ...
    i = 0
    while i < 205
    candidates_array[i].push(voting_mega_array[i])
    i += 1
    end
    mtpol = ""
     i = 0
     while i < 205     
       dist_num = test_array[i][0]
       dist_name = test_array[i][1]
       dist_party = test_array[i][2]
       dist_perc = test_array[i][3]
       if z == 0 
       mt_pol = "#mtpol"       #code for hourly #mtpol post
       else 
       mt_pol = ""
       end
        ...
        request.set_form_data(
          "status" => "At latest count: #MTHD#{dist_num}, #{dist_name} (#{dist_party}) with #{dist_perc}% of the vote #{mt_pol}",
        )
